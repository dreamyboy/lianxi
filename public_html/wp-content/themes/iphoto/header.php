	...
	<div id="header">
		<div id="header-box" class="clearfix">
			...
			<?php if (!(current_user_can('level_0'))){ ?>
				<div id="login">
				<a href="#" title="<?php _e('Log in','iphoto'); ?>"><?php _e('Log in','iphoto'); ?></a>
				...
				<div id="login-form">
					<form action="<?php echo get_option('home'); ?>/wp-login.php" method="post">
						<div id="actions">
							...
							<input type="hidden" name="redirect_to" value="<?php $page = get_page_by_title( 'post' );$page_id = $page->ID;if ($page_id!=""){ ?><?php echo get_page_link( $page_id );?><?php }?>" />
						</div>
					</form>
				</div>
			<?php } else { ?>
				<div id="logined">
					...
					<div id="info-content" class="hidden">
						<a id="info-post" href="<?php $page = get_page_by_title( 'post' );$page_id = $page->ID;if ($page_id!=""){ ?><?php echo get_page_link( $page_id );?><?php }?>" title="<?php _e('Post','iphoto'); ?>"><?php _e('Post','iphoto'); ?></a><a id="info-quit" href="<?php echo wp_logout_url( get_bloginfo('url') ); ?>" title="<?php _e('Logout','iphoto'); ?>"><?php _e('Logout','iphoto'); ?></a>
					</div>
				</div>
			<?php }?>
		</div>
